...
export default function Account() {
  ...
  useEffect(() => { if (!loading && !user) navigate('/login?redirect=/account', { replace: true }) }, [loading, user, navigate])
  useEffect(() => {
    if (!user) return
    let stored = {}
    ...
    <div className="account-layout">
      ...
      <section className="account-content"><div className="account-section-heading" id="profile"><div><span>PERSONAL DETAILS</span><h2>Profile & settings</h2><p>Keep your details ready for a faster Phone Hub checkout.</p></div></div>
        <form className="settings-card" onSubmit={save}>
          ...
          <div className="settings-divider" /><div className="settings-card-head"><div><h3>Shopping preferences</h3><p>Choose how prices are displayed across the store.</p></div></div>
